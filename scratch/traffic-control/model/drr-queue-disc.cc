...
{
  NS_LOG_FUNCTION (this);
  // m_outfile.open ("/home/guolab/LFS/NS3/test.csv");
  // freopen ("/home/guolab/LFS/NS3/drr-queue.csv", "w", stdout);
  // std::cout << "DrrEnqueueTime," << "DrrQueue,"
...
//   Ipv4Address destIp = hdr.GetDestination ();
//   uint16_t prot = hdr.GetProtocol ();
  
  // uint16_t fragOffset = hdr.GetFragmentOffset ();
  // uint16_t id = hdr.GetIdentification ();
  // std::cout << "ith packet: " << i++ << std::endl;
  // std::cout << "id------------------------------------------>" << (uint64_t)hdr.GetIdentification () << std::endl;
// // NS_LOG_DEBUG ("Packet size: " << ipv4Item->GetPacket ()->GetSize () << "src: " << src);
  
  /* test start */
  Ptr<Packet> packet = item->GetPacket ();
  UniquePacketIdTag tag;
  tag.SetUniquePacketId (9999);
  ...
  // NS_LOG_DEBUG ("Packet enqueued into flow " << h << "; flow index " << m_flowsIndices[h] << srcIp << "->" << destIp << "  prot:" << prot);
  // // NS_LOG_DEBUG ("Address " << item->GetPacket ());
//   if (prot == 6 || prot == 17)
//   {
//   std::cout << Simulator::Now ().GetMicroSeconds () << "," 
//   			<< h << "," << hdr.GetIdentification () << "," << srcIp << "," 
//   			<< destIp << "," << (uint16_t)prot << std::endl;

// // m_outfile.flush ();
// }
  	// (*m_outStream->GetStream ()).flush ();
          ...
          if ( (uint32_t) flow->GetDeficit () >= t_item->GetSize ())
            {
              ...
              // NS_LOG_DEBUG ("Dequeued packet " << item->GetPacket ());
                ...
                // Ptr<Packet> copy = item;
  // EthernetHeader eth;
//   Ipv4Header iph;
//   // copy->RemoveHeader (eth);
//   copy->PeekHeader (iph);
//   // NS_LOG_DEBUG ("Packet source IP: " << iph.GetSource () << "Payload: " << iph.GetPayloadSize () << "  Dst IP: " << copy->GetSize ());
