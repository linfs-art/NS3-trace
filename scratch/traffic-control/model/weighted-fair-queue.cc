...
TypeId WeightedFairQueue::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::WeightedFairQueue").SetParent<Queue>().AddConstructor<WeightedFairQueue>()
...
bool WeightedFairQueue::DoEnqueue(Ptr<Packet> p) {
	...
	QueueItemAlter queueItem1 = QueueItemAlter();
...
Ptr<Packet> WeightedFairQueue::DoDequeue(void) {
	...
	if (!m_wfqPQ.empty())
	{
		QueueItemAlter queueItem = m_wfqPQ.top();
