  ...
  : QueueDisc (QueueDiscSizePolicy::NO_LIMITS)
{
  ...
  // prioStream.open("/home/guolab/LFS/NS3/traffic-trace-prio.csv");

  // prioStream << "Type," << "SpBand," << "PacketUid," << "PacketId" << std::endl;
...
PrioQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
      ...
      if (item->GetPacket ()->PeekPacketTag (priorityTag))
        {
          ...
          // // NS_LOG_DEBUG ("Priority: " << priorityTag.GetPriority ());
        }
    }
  else
    {
      ...
      if (ret >= 0 && static_cast<uint32_t>(ret) < GetNQueueDiscClasses ())
        {
          band = ret;
          // std::cout << "the band is: --------------------> " << band <<std::endl;
        }
    }
  ...
  // // NS_LOG_DEBUG ("Number packets band " << band << ": " << GetQueueDiscClass (band)->GetQueueDisc ()->GetNPackets ());

  // if (prot == 6 || prot == 17)
  // {
  // std::cout << Simulator::Now ().GetMicroSeconds () << "," 
  //       << band << "," << hdr.GetIdentification () << "," << srcIp << "," 
  //       << destIp << "," << (uint16_t)prot << std::endl;
  //     }

// std::cout << "*********************************** enqueue band: " << band << std::endl;
  if (band == 7)
  {
    // std::cout << "++++++++++++++++++++++++++++++++++++++++++++ band is: " << band << std::endl;
  }
  return retval;
}
      ...
      if ((item = GetQueueDiscClass (i)->GetQueueDisc ()->Dequeue ()) != 0)
        {
          ...
          // std::cout << "Popped from band " << i << ": " << item << std::endl;
          // std::cout << "Number packets band " << i << ": " << GetQueueDiscClass (i)->GetQueueDisc ()->GetNPackets () << std::endl;
          Ptr<Ipv4QueueDiscItem> ipv4Item = DynamicCast<Ipv4QueueDiscItem> (item);
          Ipv4Header hdr = ipv4Item->GetHeader ();
