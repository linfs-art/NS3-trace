...
#ifndef WEIGHTED_FAIR_QUEUE_DISE
#define WEIGHTED_FAIR_QUEUE_DISE
...
class FqCoDelFlow2 : public QueueDiscClass {
public:
  /**
  ...
  /**
   * \brief FqCoDelFlow2 constructor
   */
  FqCoDelFlow2 ();

  virtual ~FqCoDelFlow2 ();
...
class WfqDisc : public QueueDisc {
public:
  /**
   * \brief Get the type ID.
   * \return the object TypeId
  ...
  std::map<uint32_t, uint32_t> m_weightMap;
...
private:
  ...
  std::list<Ptr<FqCoDelFlow2> > m_newFlows;    //!< The list of new flows
  std::list<Ptr<FqCoDelFlow2> > m_oldFlows;    //!< The list of old flows
...
#endif /* WEIGHTED_FAIR_QUEUE_DISE */
