...
void
FqCoDelFlow::SetQueueId (uint32_t queueId)
{
...
int32_t
FqCoDelFlow::GetQueueId (void) const
{
  NS_LOG_FUNCTION (this);
  return m_queueId;
}
    ...
    m_quantum (256)
{
  ...
  // drrStream.open("/home/guolab/LFS/NS3/traffic-trace-drr.csv");
...
FqCoDelQueueDisc::DoDequeue (void)
{
  ...
  do
    {
      bool found = false;
// std::cout << "DoDequeue do" << std::endl;
      while (!found && !m_newFlows.empty ())
        {
          // std::cout << "DoDequeue 1" << std::endl;
