...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::DRRQueueDisc::ByteLimit", UintegerValue (1000 * 1024));   //WTF?
  ...
  tchDRR.Install (devn0n2.Get (1));

  p2p.SetQueue ("ns3::DropTailQueue");
  p2p.SetDeviceAttribute ("DataRate", StringValue ("10Mbps"));
  p2p.SetChannelAttribute ("Delay", StringValue ("3ms"));
  devn1n2 = p2p.Install (n1n2);
  tchDRR.Install (devn1n2.Get (1));
  ...
  queueDiscs = tchDRR.Install (devn2n3.Get (0));
  ...
  if (flowMonitor)
    {
      ...
      // flowmon = flowmonHelper.Install (c.Get (0));
    }
