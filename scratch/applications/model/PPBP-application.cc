...
namespace ns3 {
	...
	PPBPApplication::PPBPApplication ()
	{
		...
		m_activebursts = 0;
		m_offPeriod = true;
		m_flowId = FlowIdTag::AllocateFlowId();
	}
	...
	PPBPApplication::SendPacket()
	{
		...
		// packet->AddByteTag(FlowIdTag(GetFlowId ()));
		m_txTrace (packet);
	...
	void PPBPApplication::SetFlowId (uint32_t flowId)
	{
		this->m_flowId = flowId;
	}

	uint32_t PPBPApplication::GetFlowId ()
	{
