...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
    ...
    // .AddAttribute ("UvPacketSize", "The size of packets sent in on state",
    //                StringValue ("ns3::NormalRandomVariable[Mean=600, Variance=200]"),
    //                MakePointerAccessor (&OnOffApplication::m_nvpacketsize),
...
OnOffApplication::OnOffApplication ()
  : m_socket (0),
    ...
    m_totBytes (0)
{
  m_nvpacketsize = CreateObject<NormalRandomVariable> ();
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    {
      while (1)
      {
        m_pktSize = m_nvpacketsize->GetValue (600, 40000, NormalRandomVariable::INFINITE_VALUE);
        if (m_pktSize >= 100 && m_pktSize <= 1400)
          break;
      }
